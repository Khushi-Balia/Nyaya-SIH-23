...
def signup():
    ...
    if existing_user:
        response = make_response(
            jsonify({"msg": "User already exists!Login Instead"}),
            409,
        )
        return response
...
def profile():
    ...
    try:
        ...
        return jsonify({"user": user, "msg": "User authenticated successfully"}), 200
...
def deleteprofile():
    ...
    try:
        ...
        response = jsonify({"user": user, "msg": "User deleted successfully"})
        response.set_cookie("token", "", expires=0)
        return response,200
    except jwt.ExpiredSignatureError:
        return jsonify({"msg": "Token has expired"}), 401
...
def updateprofile():
    ...
    try:
        ...
        existing_user = users.find_one({"email": email})
        user = {"name": existing_user["name"], "email": existing_user["email"]}
        response = jsonify({"user": user, "msg": "Username updated successfully"})
        return response,200
    except jwt.ExpiredSignatureError:
        return jsonify({"msg": "Token has expired"}), 401
