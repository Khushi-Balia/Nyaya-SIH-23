...
def userlogin():
    ...
    if not existing_user:
        return (
            jsonify({"success": False, "msg": "User does not exist!Signup instead!"}),
            400,
        )
...
def userprofile():
    ...
    try:
        ...
        existing_user = users.find_one({"email": email})
        if not existing_user:
            return jsonify({"success": False, "msg": "User not exist in"}), 401
...
@app.route("/users/deleteprofile", methods=["POST"])
def userdeleteprofile():
    token_cookie = request.json["token"]
    if not token_cookie:
        return jsonify({"success": False, "msg": "User not logged in"}), 401
    ...
    try:
        ...
        response = jsonify(
            {"success": True, "user": user, "msg": "User deleted successfully"}
        )
        response.set_cookie("token", "none", expires=0)
        return response, 200
    except jwt.ExpiredSignatureError:
...
def userupdateprofile():
    ...
    try:
        ...
        user = {"name": existing_user["name"], "email": existing_user["email"]}
        response = jsonify(
            {"success": True, "user": user, "msg": "Username updated successfully"}
        )
        return response, 200
    except jwt.ExpiredSignatureError:
        return jsonify({"success": False, "msg": "Token has expired"}), 401
    ...
    try:
        ...
        if check_password == 0:
            return jsonify(
                {
                    "success": False,
                    "msg": "Please try again with the correct credentials",
                }
            )
        if newpassword != confirmpassword:
            return jsonify(
